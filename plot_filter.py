# MK. 02/09/2017. First attempt using pykalman library.
...
transition_offset = [1., 2.]
observation_matrix = np.eye(2) + random_state.randn(2,2) * 0.1
observation_offset = [1.0, -1.0]
transition_covariance = np.eye(2)
observation_covariance = np.eye(2) + random_state.randn(2, 2) * 0.1
initial_state_mean = [50, -50]
...
filtered_state_estimates = kf.smooth(observations)[0]

# draw estimates
pl.figure()
lines_true = pl.plot(states, color='b')
lines_filt = pl.plot(filtered_state_estimates, color='r')
dots_meas = pl.plot(observations, 'go',)
pl.legend((lines_true[0], lines_filt[0], dots_meas[0]),
          ('true', 'filt', 'measures'),loc='lower right')
pl.show()